    ...
    Population crossoverPopulation (Population population) {
        ...
        for (int i = Main.NUMB_OF_ELITE_SCHUDELES; i < population.getListSchedule().size(); i++) {
            if (Main.CROSSOVER_RATE > Math.random()) {
                Schedule schedule1 = selectTournamentPopulation(population).sortByFitness().getListSchedule().get(0); //chon loc ngau nhien 3 ca the
    ...
    Population selectTournamentPopulation (Population population) {
        ...
        for (int i = 0; i < Main.TOURNAMENT_SELECTION_SIZE; i++) {
            tournamentPopulation.getListSchedule().set(i,
                    population.getListSchedule().get((int) (Math.random() * population.getListSchedule().size()))); //chọn vị trí ngau nhien
        }
...
//	}
    ...
    private String changeOneBitProperty(int indexProperty, String mutateStr, Data data) {
        String newStr = "";
        int randomBit = (int) (Math.random() * mutateStr.length());  //thay 1 vi tri bat kì 0->1, 1->0
